...
use window_controller::always_on_top;

pub fn main() -> iced::Result {
    #[cfg(not(target_arch = "wasm32"))]
    tracing_subscriber::fmt::init();

    iced::application(Bushido::title, Bushido::update, Bushido::view)
        .subscription(Bushido::subscription)
        .font(include_bytes!("../fonts/icons.ttf").as_slice())
        .window_size((500.0, 800.0))
        .run_with(Bushido::new);
    // sleep for 3 seconds 
    std::thread::sleep(std::time::Duration::from_secs(3));
    always_on_top("Bushido Code");
    Ok(())
}
...
impl Bushido {
    ...
    fn title(&self) -> String {
        let dirty = match self {
            Bushido::Loading => false,
            Bushido::Loaded(state) => state.dirty,
        };
            ...
            Bushido::Loading => {
                match message {
                    ...
                    Message::Loaded(Err(_)) => {
                        *self = Bushido::Loaded(State::default());
                    }
                    _ => {}
                }
    ...
    fn view(&self) -> Element<Message> {
        match self {
            Bushido::Loading => loading_message(),
            Bushido::Loaded(State {
                input_value,
                filter,
                tasks,
...
// Fonts
const ICONS: Font = Font::with_name("Iced-Bushido-Icons");
...
impl SavedState {
    fn path() -> std::path::PathBuf {
        let mut path = if let Some(project_dirs) =
            directories_next::ProjectDirs::from("rs", "Iced", "Bushido")
        {
